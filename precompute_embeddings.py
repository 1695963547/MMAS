...
class EmbeddingPrecomputer:
    ...
    def __init__(self):
        """初始化预计算器"""
        # 文件路径配置（使用相对路径，确保跨平台可移植）
        self._script_dir = Path(__file__).parent
        self.expert_knowledge_path = self._script_dir / 'dev_dataset_expert_knowledge_graph_enhanced.json'
        self.mechanism_keywords_path = self._script_dir / 'mechanism_keywords.json'
        self.semantic_model_path = 'paraphrase-multilingual-MiniLM-L12-v2'  # HuggingFace 模型名，自动下载
        
        # 输出文件路径
        self.expert_embeddings_path = self._script_dir / 'precomputed_expert_embeddings.pkl'
        self.mechanism_embeddings_path = self._script_dir / 'precomputed_mechanism_embeddings.pkl'
