...
- **六阶段稀疏证据路由框架**：从病例格式化、信息质量评估、澄清问答、知识路由、推理路由到最终决策，流程完整可复现。
...
MMAS 采用**六阶段稀疏证据路由（Six-Stage Sparse Evidence Routing）** 框架：

1. **Patient–Evaluator 交互循环**：Patient Agent 提交病例 → Evaluator Agent 评估信息完整度 → 信息不足时自动发起澄清问答（最多 D_max 轮）。
2. **DMC 知识路由**：动态多学科会诊，通过 Top-K Router 从知识库中激活最相关的医学专家。
3. **ER 推理路由**：Evidence Router 根据意图分类器选择推理路径（Diagnosis / Treatment / Knowledge / Mechanism / Ethics / Tests / Prevention）。
4. **Doctor Agent 融合推理**：综合激活专家与选定推理路径，通过加权共识（Weighted Consensus / Soft Voting）输出最终答案与置信度。

![MMAS 六阶段框架](assets/画的结果/MMAS.png)

## 方法对比

与单模型生成（Single-Model Generation）和多智能体辩论（Multi-Agent Debate）相比，MMAS 通过**知识路由（DMC）** 与 **推理路由（ER）** 的解耦设计，在保持低计算开销的同时显著提升了诊断准确率。

![方法对比](assets/图片/three_comparison.png)

## 实验结果

在 **iMEDQA** 和 **iCRAFT-MD** 两个医疗问答基准数据集上，MMAS 在 6 种主流 LLM（Qwen3-4B / DeepSeek-V3.2 / Llama-3-8B / GLM-5.1 / Qwen2.5-14B / Qwen2.5-7B）上的准确率均显著优于 MEDIQ、AI Hospital 与 MDAgents 等基线方法。

![模型性能雷达图](assets/图片/radar_model_comparison.png)
...
│   └── _chinese_lines.json           # 中文语料辅助
├── scripts/                          # 预处理脚本
│   ├── precompute_embeddings.py      # 预编码专家知识向量
...
开源模型（Qwen2.5/Qwen3/Llama 等）则通过 HuggingFace 本地加载，模型名映射见 `src/global_model_manager.py`。

## 运行方式

**入口：`src/run_optimized_mmas.py`**（请从项目根目录运行）

使用本地模型：

```bash
python src/run_optimized_mmas.py \
...
- 本仓库为项目核心代码，聚焦于三智能体协作、MoE 专家路由与诊断流水线的实现。
